...
**Sections 1-6** below provide details on the initial setup of the toolbox and guidelines for formatting the underlying datasets to analyse. **Sections 7-13** walk the user through a typical complete LotE analysis. **Section 14** provides details about files required and where to place them in the directory structure if you want to skip certain parts of the LotE analyses as metioned in the above paragraph. **Section 15** provides details about running multi-species analysis .
...
*	Running Sensitivity and adaptive capacity analyses, incuding GEAs - [Latent Factor Mixed Models (LFMM)](https://doi.org/10.1093/molbev/msz008) and [RDA (Redundancy Analysis)](https://cran.r-project.org/web/packages/vegan/vegan.pdf)
...
### 11.	Sensitivity and adaptive capacity
...
``` singularity exec ./bioconductor_3.14.sif Rscript ./-scripts-/run_LOE_sensitivity_and_adaptive_capacity.R ‘Afrixalus_fornasini’ ```

Again, to elaborate what this is doing - this will read the contents of the `run_LOE_sensitivity_and_adaptive_capacity.R` script, running through each line in sequence. Again it will pass the species name to each of the functions and run them line by line:
...
The below image provides details on the files expected by LotE if you decide to skip the relevant parts of the toolbox as mentioned above. You'd need to make sure the directory structure is as below, especially already creating directories and subdirectories as indicatd. Blue text are files you will need to add. Examples of all of these and their formats can be found [here](google.com)

 ![image](https://cd-barratt.github.io/Life_on_the_edge.github.io/vignette_figs_tables/file_requirements_if_skipping_LotE_sections.png)
